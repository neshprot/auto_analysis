...
file_name = 'log_17_12_24'
if os.path.exists(f'../outlogs/out_{file_name}'):
    merged_dict = read_and_parse_out_file(f'../outlogs/out_{file_name}')
else:
...
correlation_first_sites, correlation_second_sites = correlation_between_sites(f'../outlogs/out_{file_name}')
...
correlation_first, correlation_second, values, columns_per_mutation, unique_sites = correlation_between_muts(f'../outlogs/out_{file_name}')
...
increase_value_first = sorted(increase_value_first, key=lambda x: x[1], reverse=True)
decrease_value_first = sorted(decrease_value_first, key=lambda x: x[1], reverse=True)
increase_value_second = sorted(increase_value_second, key=lambda x: x[1], reverse=True)
decrease_value_second = sorted(decrease_value_second, key=lambda x: x[1], reverse=True)
...
make_exel_file([top_num_first_param, top_num_second_param, top_fit],
               ...
               [correlation_first, correlation_second])
os.remove('his_1.png')
os.remove('his_2.png')
os.remove('his_3.png')
